...
import { BriefcaseIcon, ChatBubbleLeftRightIcon, TrashIcon, UserPlusIcon, CheckIcon, ExclamationTriangleIcon } from '@heroicons/react/24/outline';
...
interface CandidateCartProps {
  ...
  onAcceptSuccess?: (candidateId: string) => void;
...
export default function CandidateCart({
  ...
  const handleAcceptRequest = async (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (!parentProfileId || isAccepting || isAccepted) return;

    setErrorMessage(null);
    const acceptEndpoint = `${url}/parents/${parentProfileId}/sonswhowanttobeadded/${candidateId}`;
  ...
  return (
    <div className="relative group rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-lg transition-all duration-300 ease-in-out overflow-hidden">
      ...
      <Link href={`/sons/${candidateId}`} className="block relative">
        ...
        <div className="relative w-full bg-gray-100 flex items-center justify-center overflow-hidden lg:h-100">
          <Image
            src={candidateImage}
            width={500}
            height={500}
      ...
      {addedStatus === 'request-received' && (
        <div
          className={`flex items-center justify-between border-t border-gray-100 font-bold py-4 px-5 transition-colors duration-200 ${
            isAccepted 
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h4 className={`text-lg ${isAccepted ? 'text-green-700' : 'text-blue-700'}`}>
              {isAccepting ? 'Akceptowanie...' : isAccepted ? 'Zaproszenie zaakceptowane' : 'Zaakceptuj zaproszenie'}
            </h4>
          </div>
        </div>
      )}
      ...
      {showChat && (
        <div
          className={`flex items-center justify-between border-t border-gray-100 font-bold py-4 px-5 cursor-pointer transition-colors duration-200 hover:bg-gray-50/50 ${
            isUnreadConversation ? 'text-red-700' : 'text-gray-800'
          }`}
